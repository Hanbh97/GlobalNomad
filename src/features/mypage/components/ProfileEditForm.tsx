...
const ProfileEditForm = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setImagePreview(user.profileImageUrl || null); // 초기 프로필 이미지 미리보기 세팅
    }
  ...
  const onSubmitProfileChanges = async (data: ProfileEditFormValues) => {
    try {
      ...
      if (selectedImage) {
        ...
        const current = await uploadProfileImage(formData);
        updatedProfile.profileImageUrl = current.profileImageUrl;
      }
          ...
          {...register("newPassword", {
            validate: (value) => {
              ...
              if (
                ...
                return "영문, 숫자, 특수문자를 각각 1자 이상 조합해 주세요.";
              }
              return true;
            },
          })}
